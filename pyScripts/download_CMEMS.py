...
###############################################
StartT,EndT=datenum(2022,1,1,fmt=1),datenum(2022,12,30,fmt=1) # time 
tres='monthly' # hourly, daily, monthly. 
xl=[-100,20] # longitude
yl=[-25,65] #latitude
dl=[0, 8727.917] #depth
...
if data=='ana':
   ...
   print('Downloading reanalysis data from CMEMS')
       ...
       for nn,ti in enumerate(tlist):
           ...
           furl='motuclient --motu https://nrt.cmems-du.eu/motu-web/Motu --service-id {} --product-id cmems_mod_glo_phy_my_0.083_{} --longitude-min {} --longitude-max {} --latitude-min {} --latitude-max {} --date-min "{}" --date-max "{}" --depth-min {} --depth-max {} --variable so --variable thetao --variable uo --variable vo --varibale zos --out-dir {} --out-name cmems_{}.nc --user {} --pwd {}'.format(serv,prod,pid,xl[0],xl[1],yl[0],yl[1],num2date(ti).strftime('%Y-%m-%d %H:%M:%S'),num2date(ti+dt*0.9).strftime('%Y-%m-%d %H:%M:%S'),dl[0],dl[1],sdir,st,myid,pswd)
           #print(furl)
          os.system(furl)
